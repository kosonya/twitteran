...
def main():
	...
	for nrow in xrange(len(stemmed_tweets)):
		for ncol in xrange(len(stem_lst)):
			stem, freq = stem_lst[ncol]
...
#			data = data[500:]
		...
		if mode == "density":
			...
			f_alert = open("../fetched_data/"+fname[:-4] + ".txt", "r")
			alert = ''.join(f_alert.readlines())
			f_alert.close()

			alert_tokens = tokenizer.tokenize(alert)
			alert_stems = map(stemmer.stem, alert_tokens)
			print alert_stems
			...
			for tweet in data:
				try:
					...
					if False:
						stems = [stem for stem in stems if stem in alert_stems]
